...
class OcupadasController {
    ...
    def map=[
       ...
       7:"Sabado"]
        ...
        if(Cita.executeQuery("select distinct(fecha) from Cita where status like 'abierto'")!=null){
            List ocupadasList  = Cita.executeQuery("select distinct(fecha) from Cita where status like 'abierto'")
            ...
            if (ocupadasValidacion!=null){
                List ocu = ocupadasValidacion.collect {comp -> return [title: "ocupado", start: comp.format('yyyy-MM-dd')]}
                render ocu as JSON
            }else{
